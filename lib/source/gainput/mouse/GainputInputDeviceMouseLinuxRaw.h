...
class InputDeviceMouseImplX11Raw : public InputDeviceMouseImpl
{
public:
	InputDeviceMouseImplX11Raw(InputManager& manager, InputDevice& device, InputState& state, InputState& previousState) :
		manager_(manager),
		device_(device),
		state_(&state),
