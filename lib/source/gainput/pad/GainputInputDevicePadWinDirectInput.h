...
enum class Type { BUTTON, AXIS, HAT };
...
typedef struct ControllerMapping {
    ...
    void SetDefaultMapping() {
        static const char* mapping =
            "a:b1,b:b2,y:b3,x:b0,start:b9,guide:b12,back:b8,leftstick:b10,rightstick:b11,leftshoulder:b4,rightshoulder:"
            "b5,dpup:h0.1,dpleft:"
...
enum XInputType : unsigned int { InputTypeNone, InputTypeXboxOne, InputTypeXbox360 };
...
static XInputType IsXInputDevice(const GUID* pGuidProductFromDirectInput) {
    ...
    static GUID IID_X360WiredGamepad = {
        MAKELONG(0x045E, 0x02A1), 0x0000, 0x0000, {0x00, 0x00, 0x50, 0x49, 0x44, 0x56, 0x49, 0x44}};
    static GUID IID_X360WirelessGamepad = {
        MAKELONG(0x045E, 0x028E), 0x0000, 0x0000, {0x00, 0x00, 0x50, 0x49, 0x44, 0x56, 0x49, 0x44}};
    static GUID IID_XOneWiredGamepad = {
        MAKELONG(0x045E, 0x02FF), 0x0000, 0x0000, {0x00, 0x00, 0x50, 0x49, 0x44, 0x56, 0x49, 0x44}};
    static GUID IID_XOneWirelessGamepad = {
        MAKELONG(0x045E, 0x02DD), 0x0000, 0x0000, {0x00, 0x00, 0x50, 0x49, 0x44, 0x56, 0x49, 0x44}};
    static GUID IID_XOneNewWirelessGamepad = {
        MAKELONG(0x045E, 0x02D1), 0x0000, 0x0000, {0x00, 0x00, 0x50, 0x49, 0x44, 0x56, 0x49, 0x44}};
    static GUID IID_XOneSWirelessGamepad = {
        MAKELONG(0x045E, 0x02EA), 0x0000, 0x0000, {0x00, 0x00, 0x50, 0x49, 0x44, 0x56, 0x49, 0x44}};
    ...
    /* This lets us skip RAWINPUT for popular devices. Also, we need to do this for the Valve Streaming Gamepad because
     * it's virtualized and doesn't show up in the device list. */
...
static int SortDevFunc(const void* a, const void* b) {
    ...
    if (inputA->ofs < inputB->ofs) return -1;
    if (inputA->ofs > inputB->ofs) return 1;
    return 0;
}
...
static void CreateGamePad(LPVOID pvRef) {
    ...
    HRESULT result =
        directInutGamePads->dinput->CreateDevice(gamepadInfo.gamepad.guid, &gamepadInfo.gamepadDevicePtr, NULL);

    /*get interface to gamepad*/
    result = IDirectInputDevice8_QueryInterface(gamepadInfo.gamepadDevicePtr, IID_IDirectInputDevice8,
                                                (LPVOID*)&gamepadInfo.interfacePtr);
    result = IDirectInputDevice8_Release(gamepadInfo.gamepadDevicePtr);
    /*set copperation level for shared access*/
    result = IDirectInputDevice8_SetCooperativeLevel(gamepadInfo.interfacePtr, (HWND)directInutGamePads->handle,
    ...
    result = IDirectInputDevice8_EnumObjects(gamepadInfo.interfacePtr, EnumDevObjectsCallback, &gamepadInfo,
...
static bool GetControllerGUIDFromMappingString(const char* pMapping, char* guid, int guidLen) {
    ...
    if (pFirstComma) {
        if (guidLen < (pFirstComma - pMapping + 1)) return false;
...
static char* GetControllerMappingFromMappingString(const char* pMapping) {
    ...
    if (!pFirstComma) return NULL;

    pSecondComma = strchr(pFirstComma + 1, ',');
    if (!pSecondComma) return NULL;
...
static const char* GetMappingStringForGUID(JoystickGUID guid) {
    ...
    while (mappingString) {
        ...
        if (strncmp(guidStr, dbGuidStr, sizeof(guidStr)) == 0) return mappingString;
        ++i;
...
static bool GetControllerNameFromMappingString(const char* pMapping, char* name, int nameLen) {
    ...
    pFirstComma = strchr(pMapping, ',');
    if (!pFirstComma) return false;

    pSecondComma = strchr(pFirstComma + 1, ',');
    if (!pSecondComma) return false;

    if (nameLen < (pSecondComma - pFirstComma)) return false;
...
class DirectInputInitializer {
   ...
   public:
    ...
    void DInputClean() {
        ...
        IDirectInput8_EnumDevices(gamePads.dinput, DI8DEVCLASS_GAMECTRL, EnumJoysticksCallback, &gamePads,
                                  DIEDFL_ATTACHEDONLY);
    ...
    bool Init(void* _hwnd) {
        ...
        // create instance
        HRESULT init_re = CoInitialize(nullptr);
        if (FAILED(init_re)) {
    ...
    unsigned int GetXInputType(unsigned int index) {
        if (index >= MAX_DIRECT_INPUT) return XInputType::InputTypeNone;
        unsigned int type =
            gamePads.xinputPadInfo[index].found ? gamePads.xinputPadInfo[index].type : XInputType::InputTypeNone;
        return type;
    }
...
class GainputInputDirectInputPadWin {
   ...
   public:
            ...
            if (result == size) {
                ...
                if (raw.header.dwType == RIM_TYPEHID) {
                    if (!gamepadInfo.assignedHid) {
                        ...
                        result = GetRawInputDeviceInfo(raw.header.hDevice, RIDI_DEVICENAME, (void*)gamepadInfo.name,
                                                       &bufferSize);
    ...
    int8_t TranslatePOV(DWORD value) {
        ...
        if (value >= 8) return HAT_CENTERED; /* shouldn't happen */
    ...
    bool SetRumbleEffectFeedback(const ControllerFeedback& data) {
        if (!gamepadInfo.hidDevice) {
            gamepadInfo.hidDevice = CreateFile(gamepadInfo.name, GENERIC_READ | GENERIC_WRITE,
    ...
    void Update(InputDeltaState* delta, InputState& state_, InputDevice& device_) {
        ...
        HandleButton(device_, state_, delta, PadButtonStart,
                     (gamepad.buttons[gamepad.mapping[CONTROLLER_BUTTON_START]]) != 0);
        HandleButton(device_, state_, delta, PadButtonSelect,
                     (gamepad.buttons[gamepad.mapping[CONTROLLER_BUTTON_BACK]]) != 0);
        HandleButton(device_, state_, delta, PadButtonL3,
                     (gamepad.buttons[gamepad.mapping[CONTROLLER_BUTTON_LEFTSTICK]]) != 0);
        HandleButton(device_, state_, delta, PadButtonR3,
                     (gamepad.buttons[gamepad.mapping[CONTROLLER_BUTTON_RIGHTSTICK]]) != 0);
        HandleButton(device_, state_, delta, PadButtonL1,
                     (gamepad.buttons[gamepad.mapping[CONTROLLER_BUTTON_LEFTSHOULDER]]) != 0);
        HandleButton(device_, state_, delta, PadButtonR1,
                     (gamepad.buttons[gamepad.mapping[CONTROLLER_BUTTON_RIGHTSHOULDER]]) != 0);
        HandleButton(device_, state_, delta, PadButtonL2,
                     GetAxisValue(gamepad.axes[gamepad.mapping[CONTROLLER_LEFT_TRIGGER]].value) > 0.27);
        HandleButton(device_, state_, delta, PadButtonR2,
                     GetAxisValue(gamepad.axes[gamepad.mapping[CONTROLLER_RIGHT_TRIGGER]].value) > 0.27);

        if (gamepad.nbuttons >= 13) HandleButton(device_, state_, delta, PadButton17, gamepad.buttons[13] != 0);

        HandleAxis(device_, state_, delta, PadButtonAxis4,
                   GetAxisValue(gamepad.axes[gamepad.mapping[CONTROLLER_LEFT_TRIGGER]].value));
        HandleAxis(device_, state_, delta, PadButtonAxis5,
                   GetAxisValue(gamepad.axes[gamepad.mapping[CONTROLLER_RIGHT_TRIGGER]].value));
        HandleAxis(device_, state_, delta, PadButtonLeftStickX,
                   GetAxisValue(gamepad.axes[gamepad.mapping[CONTROLLER_LEFT_X]].value));
        HandleAxis(device_, state_, delta, PadButtonLeftStickY,
                   GetAxisValue(gamepad.axes[gamepad.mapping[CONTROLLER_LEFT_Y]].value * -1L));  // its inverted??
        HandleAxis(device_, state_, delta, PadButtonRightStickX,
                   GetAxisValue(gamepad.axes[gamepad.mapping[CONTROLLER_RIGHT_X]].value));
        HandleAxis(device_, state_, delta, PadButtonRightStickY,
    ...
    void DInputBuffered(InputDeltaState* delta, InputState& state_, InputDevice& device_) {
        ...
        result    = IDirectInputDevice8_GetDeviceData(gamepadInfo.interfacePtr, sizeof(DIDEVICEOBJECTDATA), evtbuf,
                                                      &numevents, 0);
        ...
        for (i = 0; i < (int)numevents; ++i) {
            for (int j = 0; j < gamepad.hwdata.NumInputs; ++j) {
                ...
                if (evtbuf[i].dwOfs != in->ofs) continue;
    ...
    void DInputPoll(InputDeltaState* delta, InputState& state_, InputDevice& device_) {
        ...
        for (i = 0; i < gamepad.hwdata.NumInputs; ++i) {
            ...
            switch (in->type) {
                ...
                case Type::BUTTON:
                    EvaluateJoystickButton(gamepad, in->num,
                                           (UINT8)(state.rgbButtons[in->ofs - DIJOFS_BUTTON0] ? 1 : 0));
                    break;
                case Type::HAT: {
    ...
    void HandleButtonsForHat(InputDeltaState* delta, InputState& state_, InputDevice& device_, int num) {
        if (num >= gamepad.nhats) return;

        HandleButton(device_, state_, delta, PadButtonUp, (gamepad.hats[num] & gamepad.mapping.hat[CONTROLLER_HAT_UP]));
        HandleButton(device_, state_, delta, PadButtonDown,
                     (gamepad.hats[num] & gamepad.mapping.hat[CONTROLLER_HAT_DOWN]));
        HandleButton(device_, state_, delta, PadButtonLeft,
                     (gamepad.hats[num] & gamepad.mapping.hat[CONTROLLER_HAT_LEFT]));
        HandleButton(device_, state_, delta, PadButtonRight,
                     (gamepad.hats[num] & gamepad.mapping.hat[CONTROLLER_HAT_RIGHT]));
    }
