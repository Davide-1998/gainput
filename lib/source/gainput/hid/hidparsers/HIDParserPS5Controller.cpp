...
GAINPUT_COMPILE_ASSERT(sizeof(PS5Controller) <= HID_CONTROLLER_BUFFER_SIZE);
...
// GAINPUT_COMPILE_ASSERT(offsetof(PS5CInputReport, __pad2) == 41);
GAINPUT_COMPILE_ASSERT(sizeof(PS5CInputReport) == PS5_PACKET_SIZE_USB_IN);
...
GAINPUT_COMPILE_ASSERT(sizeof(PS5CEffectsState) == 47);
...
{
    GAINPUT_ASSERT(data && outSize && outOffset);
...
static int HIDUpdatePS5Controller(HIDController* controller, gainput::InputDeltaState * state)
{
    ...
    {
        GAINPUT_ASSERT(size <= PS5_PACKET_SIZE_BLUETOOTH_IN);
